...
impl JavaConfig {
    /// Creates a new JavaConfig with a given Minecraft version.
    #[allow(dead_code)]
    ...
    fn needs_user_properties(version: &str) -> bool {
        // Parse a version to determine if it's an older version that needs userProperties
        if let Some((major, minor, _)) = Self::parse_version_number_static(version) {
            // TODO: add more specific version checks
    ...
    fn needs_legacy_macos_args(version: &str) -> bool {
        // Parse a version to determine if it's an older version that needs legacy macOS args
        if let Some((major, minor, _)) = Self::parse_version_number_static(version) {
    ...
    pub fn build_command_with_executable(
        ...
        // Add natives a library path only for versions < 1.21
