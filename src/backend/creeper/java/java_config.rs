...
use crate::backend::creeper::java::version_checker::{needs_legacy_macos_args, needs_user_properties, parse_version_number};
...
impl JavaConfig {
    ...
    pub fn build_command_with_executable(
        ...
        if self.needs_natives_directory() {
            let natives_dir = minecraft_dir.join("versions").join(&self.version).join("natives");
        ...
        command
            ...
            .arg("--gameDir").arg(minecraft_dir)
            .arg("--assetsDir").arg(minecraft_dir.join("assets"))
    ...
    fn needs_natives_directory(&self) -> bool {
        self.parse_version_number().map(|v| v < (1, 21, 0)).unwrap_or(true)
    }
